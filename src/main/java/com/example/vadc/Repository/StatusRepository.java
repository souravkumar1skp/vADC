...
public interface StatusRepository extends JpaRepository<EventMaster, Integer> {
    ...
    @Query(value = "with my_cte as (\n" +
            ...
            "),\n" +
            "cte2 as (\n" +
            "select count(*) as completed from my_cte where candidate_event_status= 'COMPLETED' and start_date>=:startDate and end_date<=:endDate\n" +
            ")\n" +
            "select * , (select count(*) from my_cte where start_date>=:startDate and end_date<=:endDate) as total from cte2 ", nativeQuery = true)
